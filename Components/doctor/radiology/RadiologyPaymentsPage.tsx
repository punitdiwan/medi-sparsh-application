...
import { RadiologyPaymentPDF } from "./RadiologyPaymentPDF";
import { RadiologyPaymentsTablePDF } from "./RadiologyPaymentsTablePDF";
// import { RadiologyPaymentPDF } from "./RadiologyPaymentPDF";
// import { RadiologyPaymentsTablePDF } from "./RadiologyPaymentsTablePDF";
...
export default function RadiologyPaymentsPage({ payments: initialPayments }: RadiologyPaymentsPageProps) {
    ...
    const columns = useMemo(() => {
        const allCols: ColumnDef<RadiologyPayment>[] = [
            {
                header: "Bill ID",
                accessorKey: "billNo",
                cell: ({ row }) => (
    ...
    const handleBulkExport = async () => {
        try {
            ...
            const pdfDoc = (
                <RadiologyPaymentsTablePDF
                    payments={filteredPayments}
                    title={`Radiology Payments - ${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}`}
                />
            );
            ...
            const link = document.createElement("a");
            link.href = url;
            link.download = `radiology-payments-${activeTab}-${new Date().toISOString().split("T")[0]}.pdf`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            URL.revokeObjectURL(url);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-Module-header p-6 rounded-2xl">
                <div className="space-y-1.5">
                    ...
                    <p className="text-gray-300 flex items-center gap-2 text-sm font-medium pl-10">
                        Manage and track all radiology diagnostic financial transactions
